...
import { listar as listarEnvios } from 'src/services/enviosService';
import { consultar as consultarEnvio, registrarAvance } from 'src/services/seguimientoService';
...
async function cargarActivos() {
  try {
    ...
    enviosActivos.value = lista.filter(e => e.estadoId !== 4);
  } catch { /* silencioso, es solo sugerencia */ }
}
...
async function buscar() {
  ...
  try {
    const enc = await consultarEnvio(codigo);
    envio.value = enc;
    nuevoEstadoId.value = enc.estadoId;
    observacion.value = '';
  } catch (err) {
    if (err.response && err.response.status === 404) {
      mensajeError.value = `No se encontro ningun envio con el codigo "${codigo}"`;
    } else {
      mensajeError.value = extractError(err);
    }
  }
}
...
async function guardarAvance() {
  ...
  try {
    const actualizado = await registrarAvance(envio.value.codigo, {
      id_estado: nuevoEstadoId.value,
      observacion: observacion.value
    });
    envio.value = actualizado;
    nuevoEstadoId.value = actualizado.estadoId;
    observacion.value = '';
