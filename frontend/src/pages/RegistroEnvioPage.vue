    ...
    >
      ...
      <q-step :name="4" title="Confirmar" icon="check_circle">
        ...
        <q-stepper-navigation>
          ...
          <q-btn color="positive" icon="check" label="Confirmar y registrar" :loading="guardando" no-caps @click="registrar" />
        </q-stepper-navigation>
      </q-step>
    </q-stepper>
...
import { CIUDADES, TIPOS_CARGA } from 'src/data/mock';
import { crear as crearEnvio } from 'src/services/enviosService';
...
async function registrar() {
  ...
  try {
    const payload = {
      remitente:    { nombre: form.remitenteNombre,    documento: form.remitenteDoc,    telefono: form.remitenteTel },
      destinatario: { nombre: form.destinatarioNombre, documento: form.destinatarioDoc, telefono: form.destinatarioTel },
      origen: form.origen,
    ...
    const nuevoEnvio = await crearEnvio(payload);
